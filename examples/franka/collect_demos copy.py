...
def main(cfg: DictConfig):
    ...
    with robot_client_context(server_ip, port, FrankaClient) as client:
        ...
        for i, keypoint in enumerate(keypoints):
            ...
            camera.start()
            data = data + collect_data_during_movement(
                client=client,
        ...
        frame_height, frame_width, _ = data[0]['images']['cam_0']['color'].shape
        ...
        for i, frame_data in enumerate(data):
            color_frame = frame_data['images']['cam_0']['color']
...
if __name__ == '__main__':
        ...
        with open(f'/home/yiyang/hsc/ogvla_realworld/data/train/ewe_trajectory_1.pkl', 'rb') as f:
            data = pickle.load(f)            
        ...
        # Create a video writer
        video_path = f'/home/yiyang/hsc/ogvla_realworld/data/train/ewe_trajectory_1.mp4'
        
        frame_height, frame_width, _ = data[0]['images']['cam_0']['depth'].shape
        fps = 30  # Frames per second
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        video_writer = cv2.VideoWriter(video_path, fourcc, fps, (frame_width, frame_height))
        ...
        # Write frames to the video
        for i, frame_data in enumerate(data):
            color_frame = frame_data['images']['cam_0']['depth']
            color_frame = cv2.cvtColor(color_frame, cv2.COLOR_GRAY2BGR)
            color_frame = cv2.applyColorMap(color_frame, cv2.COLORMAP_JET)
            video_writer.write(color_frame)
            print(f"Writing frame {i + 1}/{len(data)}")
